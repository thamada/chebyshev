...
void chebyshev_coefficients(double (*f)(double), double a, double b, int n, double* coeff) {
    for (int k = 0; k <= n; k++) {
        double sum = 0.0;
        for (int i = 0; i < n; i++) {
            double x_i = cos(M_PI * (2 * i + 1) / (2.0 * n));
            double x_mapped = 0.5 * (b - a) * (x_i + 1) + a;
            sum += f(x_mapped) * cos(k * M_PI * (2 * i + 1) / (2.0 * n));
        }
        coeff[k] = (2.0 / n) * sum;
    }
...
int main() {
    ...
    // チェビシェフ係数を計算
    chebyshev_coefficients(func, a, b, N+1, coeff);
    ...
    for (double x = a; x <= b; x += 1.0/1024.0) {
      double z_cheby = chebyshev_interpolate(x, a, b, N+1, coeff);
